...
class OpenAICompatibleClient :
    def __init__(self,model:str,api_key:str,base_url):
        self.model = model
...
for i in range(5):  # 设置最大循环次数
    ...
    """
    Thought:.*?Action:.*?
    非贪婪匹配从 Thought: 开始，到 Action: 内容结束的一整段；
    (?=\n\s*(?:Thought:|Action:|Observation:)|\Z) 正向预查
    匹配终止条件：下一行出现新的 Thought/Action/Observation，或者文本末尾，直接截断。
"""
